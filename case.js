...
const hentai = require('./lib/scraper/hentai');
...
const audione = JSON.parse(fs.readFileSync('./media/database/audio.json'));
const stikerne = JSON.parse(fs.readFileSync('./media/database/sticker.json'));
const imagene = JSON.parse(fs.readFileSync('./media/database/image.json'));
const videone = JSON.parse(fs.readFileSync('./media/database/video.json'));
...
module.exports = haruka = async (haruka, m, msg, chatUpdate, store) => {
	try {
		...
		let id = '120363190404312751@g.us';
			...
			for (const [sholat, waktu] of Object.entries(jadwalSholat)) {
				if (timeNow === waktu) {
					haruka.autosholat[m.chat] = [
						haruka.sendMessage(m.chat, {
							audio: {
								url: "https://github.com/PhysicalAbility/Media/raw/master/audio.mp3"
							},
							mimetype: 'audio/mpeg',
							contextInfo: {
								mentionedJid: [m.sender],
		...
		switch (command) {		
			...
			case 'backup': {
				...
				let date = new Date();
				let filename = `backup_${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getDate().toString().padStart(2, '0')}_${date.getHours().toString().padStart(2, '0')}-${date.getMinutes().toString().padStart(2, '0')}-${date.getSeconds().toString().padStart(2, '0')}`;
			...
			case 'intro': {
				const intro = `🌟 *「 INTRODUCE 」* 🌟\n\n` +
					...
					`🕌 *Agama:* \n` +
					`💬 *Status:* \n\n` +
					`✨ Yuk kenalan biar makin akrab, kak! 🤗💕`;
				m.reply(intro);
			};
		break;

			case 'gamau': {
				const gamau = `😿 Yaudah deh kak, Mora nggak maksa kok... Tapi kalau berubah pikiran, panggil Mora lagi ya! 🤭💕`;
				m.reply(gamau);
			};
			break;
			...
			case 'qc':{
				...
				const avatar = await haruka.profilePictureUrl(m.quoted ? m.quoted.sender : sender, "image").catch(() => 'https://i.ibb.co.com/p2nKgqP/image.png')
				const json = {
					type: "quote",
					format: "png",
			...
			case 's': case 'sticker': case 'stiker': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!quoted) return newReply(`Kirim/Reply Gambar/Video dengan caption ${prefix + command}\nDurasi video maksimal 9 detik!`);
				...
				if (/image/.test(mime)) {
					...
					newReply(`Kirim atau reply gambar/video dengan caption ${prefix + command}`);
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			// Fix Get Feature, Anti Penyusup
			case 'fetch': case 'get': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text.startsWith('http')) return newReply(`No Query?\n\nExample : ${prefix + command} https://www.google.com`);
				try {
					const res = await axios.get(isUrl(text) ? isUrl(text)[0] : text);
					if (!isCreator && !/text\/html/.test(res.headers['content-type'])) {
						return newReply('Maaf, hanya konten HTML yang diizinkan untuk di-fetch.');
					}
			...
			case 'addvideo':{
				...
				if (videone.includes(q)) return newReply("The name is already in use")
				let delb = await haruka.downloadAndSaveMediaMessage(quoted)
				videone.push(q)
				await fsx.copy(delb, `./media/${q}.mp4`)
				fs.writeFileSync('./media/database/video.json', JSON.stringify(videone))
				fs.unlinkSync(delb)
				newReply(mess.done);
			}
			break;
			...
			case 'delvideo':{
				...
				if (!videone.includes(q)) return newReply("The name does not exist in the database")
				let wanu = videone.indexOf(q)
				videone.splice(wanu, 1)
				fs.writeFileSync('./media/database/video.json', JSON.stringify(videone))
			...
			case 'listvideo':{
				...
				teks += `│\n└────────────⭓\n\n*Totally there are : ${videone.length}*`
				newReply(teks)
			}
			break;
			...
			case 'addimage':{
				...
				if (imagene.includes(q)) return newReply("The name is already in use")
				let delb = await haruka.downloadAndSaveMediaMessage(quoted)
				imagene.push(q)
				await fsx.copy(delb, `./media/${q}.jpg`)
				fs.writeFileSync('./media/database/image.json', JSON.stringify(imagene))
				fs.unlinkSync(delb)
				newReply(mess.done);
			}
			break;
			...
			case 'delimage':{
				...
				if (!imagene.includes(q)) return newReply("The name does not exist in the database")
				let wanu = imagene.indexOf(q)
				imagene.splice(wanu, 1)
				fs.writeFileSync('./media/database/image.json', JSON.stringify(imagene))
			...
			case 'listimage':{
				...
				teks += `│\n└────────────⭓\n\n*Totally there are : ${imagene.length}*`
				newReply(teks)
			}
			break;
			...
			case 'addsticker':{
				...
				if (stikerne.includes(q)) return newReply("The name is already in use")
				let delb = await haruka.downloadAndSaveMediaMessage(quoted)
				stikerne.push(q)
				await fsx.copy(delb, `./media/${q}.webp`)
				fs.writeFileSync('./media/database/sticker.json', JSON.stringify(stikerne))
				fs.unlinkSync(delb)
				newReply(mess.done);
			}
			break;
			...
			case 'delsticker':{
				...
				if (!stikerne.includes(q)) return newReply("The name does not exist in the database")
				let wanu = stikerne.indexOf(q)
				stikerne.splice(wanu, 1)
				fs.writeFileSync('./media/database/sticker.json', JSON.stringify(stikerne))
			...
			case 'liststicker':{
				...
				teks += `│\n└────────────⭓\n\n*Totally there are : ${stikerne.length}*`
				newReply(teks)
			}
			break;
			...
			case 'addvn':{
				...
				if (audione.includes(q)) return newReply("The name is already in use")
				let delb = await haruka.downloadAndSaveMediaMessage(quoted)
				audione.push(q)
				await fsx.copy(delb, `./media/${q}.mp3`)
				fs.writeFileSync('./media/database/audio.json', JSON.stringify(audione))
				fs.unlinkSync(delb)
				newReply(mess.done);
			}
			break;
			...
			case 'delvn':{
				...
				if (!audione.includes(q)) return newReply("The name does not exist in the database")
				let wanu = audione.indexOf(q)
				audione.splice(wanu, 1)
				fs.writeFileSync('./media/database/audio.json', JSON.stringify(audione))
			...
			case 'listvn':{
				...
				teks += `│\n└────────────⭓\n\n*Totally there are : ${audione.length}*`
				newReply(teks)
			}
			break;
			...
			case 'ytsearch': {
				...
				try {
					...
					let results = search.videos.slice(0, 5); // Ambil 5 hasil pertama
			...
			case 'ytmp3': {
				...
				try {
					...
					let audioData = await saveTube.dl(url, 3, 'audio'); // kualitas audio 128 kbps
			...
			case 'ytv': {
				...
				try {
					...
					// Menggunakan scraper lokal
					let videoData = await saveTube.dl(url, 5, 'video'); // kualitas video 720p
				...
				for (let message of selectedImages) {
					push.push({
						...
						header: proto.Message.InteractiveMessage.Header.fromObject({
							title: `Gambar - ${i++}`,
							hasMediaAttachment: true,
							imageMessage: await createImage(message)
						}),
						nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
							buttons: [
								{
									"name": "cta_url",
									"buttonParamsJson": `{"display_text":"View Source 👀","url":"https://www.pinterest.com/search/pins/?rs=typed&q=${text}","merchant_url":"https://www.pinterest.com/search/pins/?rs=typed&q=${text}"}`
								}
							]
						})
					});
			...
			case 'store': {
				try {
					...
					sendButtonImage(m.chat, '', teks, { url: thumbUrl }, button, m)
				} catch (error) {
					console.log(error);
				}
			}
							...
							interactiveMessage: proto.Message.InteractiveMessage.create({
								...
								nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
									buttons: [
										...
										{
											"name": "cta_url",
											"buttonParamsJson": `{\"display_text\":\"👑 Owner's Profile\",\"url\":\"https://api.whatsapp.com/send?phone=${ownerNumber}\",\"merchant_url\":\"https://www.google.com\"}`
										}
									],
								}),
								contextInfo: {
					...
					buttons: [
						{
							...
							buttonText: {
								displayText: "Accept Menfess"
							}
						},
						{
							buttonId: `${prefix}tolakmenfess`,
							buttonText: {
								displayText: "Reject Menfess"
							}
						}
					],
					viewOnce: true,
			...
			case 'addusradmin': {
				...
				try {
					...
					let userExists = (await onWhatsApp(userToAdd.split`@`[0]))[0] || {};
			...
			case 'addusr': {
				...
				let onWA = await onWhatsApp(users);
			...
			case '50gb': {
				...
				try {
					...
					if (!userData.errors) {
						...
						sendButtonImage(users, '', teks2, thumb, button, m);
					} else {
